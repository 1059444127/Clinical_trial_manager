	...
	if(isset($_POST['action']) && $_POST['action'] == 'edit_hospital'){
		...
		if( user_can('edit_hospital') && can_access('hospital', $hospital_id) ):
			...
			$result = $db->update(TBL_HOSPITALS,
			    ...
			    $q1 = "SELECT * FROM tbl_hospitals WHERE ID = $hospital_id";
                $r1 = $db->get_results($q1);
                foreach($r1 as $a):
                    $b = unserialize($a->trials);
        $name1 = $a->name;
                        foreach($b as $t):
                            $sql = "SELECT * FROM tbl_done WHERE TrialID = $t AND HospitalID = $hospital_id";
                            $req = $db->query($sql);
                            if ($req<1){
                              $sql = "INSERT INTO tbl_done (TrialID, HospitalID) VALUES ($t,$a->ID)";
                              $don = $db->query($sql);
                            }      
                        endforeach;
                endforeach;
			if($result):
				$notification_args = array(
					...
					'notification' => 'You have successfully updated hospital ('.$name1.').',
				);
